...
export class SubcontractorService {
 ...
 getSubcontractors(onlyActive: boolean = false): Observable<Subcontractors[]> {
  ...
  return from(this.getHeaders()).pipe(
    switchMap((headers) =>
      this.http.get<{ data: Subcontractors }>(`${this.apiURL}/Subcontractor/${id}`, { headers }),
    ),
    map((res) => res.data) // ✅ Return the whole data object
  );
}
 ...
 getSubcontractorWorkItemMappings(onlyActive: boolean = false): Observable<any[]> {
  ...
  updateIsActive(subcontractorId: string, isActive: boolean): Observable<any> {
    return from(this.getHeaders()).pipe(
      switchMap((headers) =>
        this.http.post(`${this.getsubcontractor}/${subcontractorId}/${isActive}`, null, {
          headers,
        }),
      ),
    );
